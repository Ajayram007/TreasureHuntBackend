  ...
  try {
    ...
    if (user.role === "admin") {
      return res.status(403).json({
        message: "Admins cannot play the game. Only users can start the game."
      });
    }
    ...
    const path = levels.map(level => {
      const randomPlace =
        level.places[Math.floor(Math.random() * level.places.length)];
      return { levelNumber: level.levelNumber, place: randomPlace };
    });
...
router.post("/verify-qr/:playerId", async (req, res) => {
  try {
    const playerId = req.params.playerId;  
    const { levelNumber, place } = req.body;  
